/*
 * FILE       : map_3p1_value_characteristic.c
 *
 * PROGRAMMERS: Kevin Wertman (SAIC)
 *
 * DESCRIPTION:
 *     This program takes a series of EDCS 3.1 EV codes or labels
 *     and prints out the mapping information to map to EDCS 4.x
 *
 * Usage:
 *   map_3p1_value_characteristic code1 label2 code3 label4 ... codeN-1 labelN
 *
 * Options: none
 *
 * examples:
 *
 *   To find mapping information for EVC_MISSING:
 *     map_3p1_value_characteristic MISSING
 *   or
 *     map_3p1_value_characteristic 2
 *
...
const char *const ToolName = "map_3p1_value_characteristic";
...
)
{
    ...
    fprintf(stderr,
"map_3p1_value_characteristic takes a series of EDCS 3.1 EV codes or labels"\
 "and prints out the mapping information to map to EDCS %u.%u.\n"\
"\n"\
"Usage: map_3p1_value_characteristic code1 label2 code3 label4 ... codeN-1 labelN\n"\
"\n"\
" Options:\n" \
"\n" \
"  None\n" \
...
{
    const EDCS_3p1_4px_Characteristic_Mapping_Structure *mapping_info_out_ptr = NULL;
    ...
    else
    {
        for (i = 1; i < argc; i++)
        {
            if ((ev_code = strtol(argv[i], NULL, 10)) != 0)
            {
                status = EDCS_Map3p1CharacteristicCode(ev_code,
                                                       &mapping_info_out_ptr);
            }
            else
            {
                status = EDCS_Map3p1CharacteristicLabel(argv[i],
                                                        &mapping_info_out_ptr);
            }
            ...
            {
                /*
                 * the map_type will be only one of 4 things for EV mapping:
                ...
                switch (mapping_info_out_ptr->map_type)
                {
                    ...
                    {
                        printf("EDCS 3.1 EV %s maps to no EDCS 4.x concept. "\
                               "The concept was deleted.\n",
                               mapping_info_out_ptr->ev_3p1_label);
                        break;
                    }
                    case EDCS_MAPPING_TYP_ONE_TO_ONE:
                    {
                        printf("EDCS 3.1 EV %s maps exactly to "\
                               "the EDCS 4.x EV symbolic constant %s\n",
                               mapping_info_out_ptr->ev_3p1_label,
                               EDCS_PrintValueCharacteristicCode
                               (mapping_info_out_ptr->ev_4px_code));

                        break;
                    }
                    case EDCS_MAPPING_TYP_ONE_TO_MANY:
                    {
                        if (mapping_info_out_ptr->ev_4px_code != 0)
                        {
                            printf("EDCS 3.1 EV %s has a one to many mapping,"\
                              " with a default mapping to "
                              "EDCS 4.x EV code %s.\n",
                              mapping_info_out_ptr->ev_3p1_label,
                              EDCS_PrintValueCharacteristicCode
                              (mapping_info_out_ptr->ev_4px_code));
                    ...
                    {
                        printf("EDCS 3.1 EV %s has a special case mapping "\
                          "to EDCS 4.x EV code %s.\n",
                          mapping_info_out_ptr->ev_3p1_label,
                          EDCS_PrintValueCharacteristicCode
                          (mapping_info_out_ptr->ev_4px_code));
                        break;
                    ...
                    default:
                    {
                        fprintf(stderr, "Error: EDCS 3.1 code %s has "\
                          "an unhandled mapping type %d\n",
                          argv[i], mapping_info_out_ptr->map_type);
                        break;
                    }
                }
            ...
            {
                fprintf(stderr, "The call to map EDCS 3.1 EV %s "\
